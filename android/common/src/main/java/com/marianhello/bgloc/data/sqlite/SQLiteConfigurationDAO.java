...
public class SQLiteConfigurationDAO implements ConfigurationDAO {
  ...
  private Config hydrate(Cursor c) throws JSONException {
    ...
    config.setOngoing(c.getInt(c.getColumnIndex(ConfigurationEntry.COLUMN_NAME_ONGOING)) == 1);
    config.setPaused(c.getInt(c.getColumnIndex(ConfigurationEntry.COLUMN_NAME_PAUSED)) == 1);
    return config;
  }
